...
import { Form, Input, Button, Modal, message, Select, DatePicker, Card, Spin } from 'antd';
        ...
        <>
            <Modal open={visible} onCancel={onClose} footer={null}>
                <Spin spinning={loading}>
                    <Form form={form} onFinish={submitData} requiredMark={false} className="content" layout="vertical">
                        <div className="flower">
                            ...
                            <Card title="Deal Information">
                                ...
                                <Form.Item label="Stage" name="stageId" rules={[{ required: true, message: 'Please select a stage!' }]}>
                                    <Select>
                                        {stages.map(stage => (
...
const getStages = async () => {
    const response = await axios.get('https://api.pipedrive.com/v1/stages?api_token=89a00ee329df732768497bd630d0a4ca97ef4205');
    return response.data.data;
};

const getUsers = async () => {
    const response = await axios.get('https://api.pipedrive.com/v1/users?api_token=89a00ee329df732768497bd630d0a4ca97ef4205');
    return response.data.data;
};
